...
public class ComputerPower : MonoBehaviour {
	...
	{	if ((Time.time > currenttime)&&!Done) {
		...
		if (TasksDone == 1) {
			SequenceText.GetComponent<Text> ().text = "Power it down! Type: " + sequence[0] + " To turn off the Computer!";
		}
		if (TasksDone == 2) {
			SequenceText.GetComponent<Text>().text="Turn it back on! Type: " + sequence[0] + " To power on the Computer!";
		}
		if (TasksDone == 3) {
